...
def _dedup_secrets(secrets: list) -> list:
    """
    Deduplicate by (type, match). When the same credential appears in multiple
    JS files keep only the first occurrence.
    """
    seen = set()
    out  = []
    for s in secrets:
